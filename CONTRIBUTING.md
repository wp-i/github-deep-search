...
Use the same workflow for new features, bug fixes, debugging, and tests: trace -> locate the earliest incorrect stage -> make the minimal root-cause change -> verify with real tests. Do not add temporary compensation, downstream rescue branches, or test-only rewrites to make one example pass.
