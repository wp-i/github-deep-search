...
def _system_prompt(role: str) -> str:
    return (
        ...
        "Apply the report's evidence semantics correctly: an evidence-backed adjacent lead may be retained at a low "
        "score while its core requirement remains explicitly unconfirmed; that is not a contradiction by itself. "
        "Unknown means the supplied material neither confirms nor explicitly denies a capability, so do not demand "
        "missing or unsupported without explicit repository evidence of absence. A verified capability may be narrower "
        "than the user's core requirement and must be judged only against its cited excerpt, not treated as a claim "
        "that the entire core requirement is supported. A clear runtime-behavior statement in the repository's own "
        "README or description is valid evidence for that narrow claim; do not demand source-code proof merely because "
        "the evidence is self-authored. However, if that statement includes a limitation, the displayed capability must "
        "retain the limitation when omitting it would make the claim unconditional. Flag an issue only when a displayed "
        "claim exceeds its evidence, "
        "the tier contradicts the evidence state, a meaningful surviving lead was omitted, or the report cannot support "
        "a user decision. "
