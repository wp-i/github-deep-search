...
| 结构化 `EvidenceReference` | `EvidenceCoverage` 现以附加、兼容字段记录 README/路径/源码或候选身份观察的类型、定位、受限摘录、当前请求别名与可用行号；JSON 已投影，原有字符串证据与证据门判定未变 | 为支持性主张补充可渲染的引用入口，并以更完整的场景矩阵验证 README、路径和源码三类局部定位 |
...
3. 为支持性主张提供可渲染的 `EvidenceReference` 入口，并完成 README/路径/源码定位的真实场景覆盖。
